...
static int jo_processDU(FILE* fp, int& bitBuf, int& bitCnt, int DC, const unsigned short HTDC[256][2], const unsigned short HTAC[256][2], std::ifstream& DCT_DUMP) {
...
bool jo_write_jpg(const char* filename, int width, int height, int quality) {
	...
	std::ifstream DCT_DUMP = std::ifstream("dctDump", std::ios::binary);

	if (subsample) {
		for (int y = 0; y < height; y += 16) {
			for (int x = 0; x < width; x += 16) {
				DCY = jo_processDU(fp, bitBuf, bitCnt, DCY, YDC_HT, YAC_HT, DCT_DUMP);
				DCY = jo_processDU(fp, bitBuf, bitCnt, DCY, YDC_HT, YAC_HT, DCT_DUMP);
				DCY = jo_processDU(fp, bitBuf, bitCnt, DCY, YDC_HT, YAC_HT, DCT_DUMP);
				DCY = jo_processDU(fp, bitBuf, bitCnt, DCY, YDC_HT, YAC_HT, DCT_DUMP);
				//// subsample U,V
				DCU = jo_processDU(fp, bitBuf, bitCnt, DCU, UVDC_HT, UVAC_HT, DCT_DUMP);
				DCV = jo_processDU(fp, bitBuf, bitCnt, DCV, UVDC_HT, UVAC_HT, DCT_DUMP);
			}
		}
	}
	else {
...
void EncodeJPEG(std::string oFile)
{
	...
	int height;
	int quality;
	std::ifstream headers = std::ifstream("headers", std::ios::binary);
	headers >> width;
	headers >> height;
	headers >> quality;
	headers.close();
	jo_write_jpg(oFile.c_str(), width, height, quality);
}